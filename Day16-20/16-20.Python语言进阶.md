...
- `itertools`模块
  ...
  itertools.cycle(('A', 'B', 'C'))
   ...
   ```Python
  itertools 是 Python 标准库中的一个模块，提供了许多用于高效循环和迭代操作的函数。这些函数可以用于创建迭代器，以更加简洁和高效的方式处理复杂的数据结构。以下是一些常用的 itertools 函数及其简要说明：

  1、infinite_iterators（无限迭代器）
  count(start=0, step=1): 返回一个从 start 开始，每次增加 step 的无限迭代器。
  cycle(iterable): 循环遍历输入的迭代器，生成一个无限长的迭代器。
  repeat(object, times=None): 重复一个对象 times 次，如果 times 为 None，则无限重复。
  
  2、iterators_terminating_on_the_shortest_input_sequence（根据最短输入序列终止的迭代器）
  accumulate(iterable, func=operator.add): 返回累积值，初始值为输入序列的第一个元素，func 用于指定累积方式（默认为加法）。
  chain(*iterables): 将多个迭代器串联成一个迭代器。
  compress(data, selectors): 返回 data 中所有对应 selectors 为 True 的元素。
  dropwhile(predicate, iterable): 丢弃 iterable 中满足 predicate 的元素，直到不满足为止，然后返回剩下的元素。
  filterfalse(predicate, iterable): 返回 iterable 中不满足 predicate 的元素。
  groupby(iterable, key=None): 将连续的、具有相同 key 值的元素分组。
  islice(iterable, start, stop[, step]): 返回 iterable 的一个切片，相当于列表切片 [start:stop:step]。
  starmap(function, iterable): 类似于 map()，但 iterable 中的每个元素是一个参数元组，function 将这些参数解开后调用。
  takewhile(predicate, iterable): 返回 iterable 中满足 predicate 的元素，直到不满足为止。
  tee(iterable, n=2): 返回 n 个独立的迭代器，每个迭代器都包含 iterable 的所有元素。
  zip_longest(*iterables, fillvalue=None): 将多个迭代器压缩成一个迭代器，对于最短的迭代器，用 fillvalue 填充缺失的值。
  
  3、combinatoric_generators（组合生成器）
  product(*iterables, repeat=1): 计算多个迭代器的笛卡尔积。
  permutations(iterable, r=None): 返回 iterable 中所有长度为 r 的排列。
  combinations(iterable, r): 返回 iterable 中所有长度为 r 的组合。
  combinations_with_replacement(iterable, r): 返回 iterable 中允许重复元素的长度为 r 的组合。
  ...
  # islice  
  for i in itertools.islice('ABCDEFG', 2, 5):  
      print(i)  # 输出: C, D, E  
    
  # takewhile  
  for i in itertools.takewhile(lambda x: x < 5, [1, 4, 3, 9, 6]):  
      print(i)  # 输出: 1, 4, 3  
...
- `collections`模块
  ...
  1. namedtuple
  功能：创建一个具有命名字段的元组类。
  特点：可以通过字段名访问元组的元素，而不是使用索引，这增强了代码的可读性。
  示例：
  ...
  2. deque
  功能：双端队列，可以在两端高效地添加和删除元素。
  特点：比列表在频繁插入和删除元素时性能更好，并且支持线程安全的操作。
  示例：
  ...
  3. defaultdict
  功能：默认字典，当访问不存在的键时，会返回一个默认值，而不会抛出KeyError异常。
  特点：为字典的键提供了默认值功能，避免了KeyError并简化了代码。
  示例：
  ...
  特点：可以按照添加的顺序遍历字典。
  示例：
  虽然OrderedDict没有直接的示例代码，但可以理解为它在保持元素插入顺序方面与普通的dict不同。

  5. Counter
  功能：计数器，用于统计可哈希对象的频率。
  特点：可以快速计算一个序列中每个元素出现的次数。
  示例：
  from collections import Counter  
  ...
  print(word_count['grape'])  # 输出: 0（未出现的元素计数为0）

  6. ChainMap
  功能：可以将多个字典合并成一个逻辑上的字典。
  特点：使得可以在多个字典上执行操作，而不需要复制数据。
  示例：
