...
class Test:
    ...
    def __init__(self, foo):      #__init__ 是类的构造函数（或初始化方法），在创建类的实例时自动调用。
        self.__foo = foo          #接受一个参数 foo 并将其赋值给实例的私有属性 __foo;__foo 是一个私有属性，因为其名称以双下划线开头。这意味着它不应该从类的外部直接访问。

    def __bar(self):              #__bar 是一个私有方法，同样因为名称以双下划线开头;它打印私有属性 __foo 的值，然后打印字符串 '__bar'
        print(self.__foo)
        print('__bar')
...
class Point(object):
    ...
    def distance_to(self, other):       #other 也是一个 Point 对象，至少具有 x 和 y 属性
        """计算与另一个点的距离
...
def main():
    ...
    p2.move_by(-1, 2)
    print(p2)
    print(p1.distance_to(p2))         #调用p1对象的distance_to方法，并将p2作为参数传递给这个方法；
